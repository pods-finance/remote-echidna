...
for instance_id in $(aws s3 ls s3://$S3_BUCKET/4_FINISHED/ | awk '{print $NF}'); do
  echo $instance_id
  aws s3 cp aws s3 ls s3://$S3_BUCKET/4_FINISHED/$instance_id .
  DATE=$(grep 'date:' $instance_id); DATE=${DATE//*date: /}; 
  PROJECT_GIT_CHECKOUT=$(grep 'project_git_checkout:' $instance_id); PROJECT_GIT_CHECKOUT=${PROJECT_GIT_CHECKOUT//*project_git_checkout: /}; 
